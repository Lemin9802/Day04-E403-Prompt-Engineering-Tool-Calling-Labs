...
def _normalize(text: str) -> str:
    """Normalize text for simple Vietnamese/English product search."""
...
class OrderDataStore:
    """
    Data access and deterministic business logic for the OrderDesk lab.

    Responsibilities:
    - load products.json
    - search catalog
    - return product details
    - create deterministic detail tokens
    - create deterministic discounts
    - calculate totals
    - save final order JSON
    """
    ...
    def __init__(self, data_dir: Path, output_dir: Path, *, today: str | None = None) -> None:
        ...
        self.product_index: dict[str, ProductRecord] = {
            item.product_id: item for item in self.products
        }
    ...
    def build_detail_token(product_ids: list[str]) -> str:
        """Build a deterministic validation token for a product set."""
    ...
    def validate_detail_token(self, product_ids: list[str], detail_token: str) -> bool:
        """Validate that the detail token matches the requested product set."""
    ...
    def list_products(
        ...
        *,
        ...
        """
        Search products by name, brand, category, tags, and description.

        Return compact summaries only. Exact prices and stock should be obtained
            ...
            results.append(
                (
                    ...
                    product.product_id,
                    {
                        ...
                        "next_step": (
                            "Call get_product_details with the selected product_id list "
                            "to verify price, stock, warranty, and detail_token."
                        ),
                    },
                )
            )
    ...
    def get_product_details(self, product_ids: list[str]) -> dict:
        ...
        The detail token must be passed into pricing and saving tools.
        """
        details: list[dict] = []
        ...
        found_product_ids = [
            item["product_id"] for item in details if item.get("status") == "ok"
        ]
    ...
    def get_discount(self, *, seed_hint: str, customer_tier: str = "standard") -> dict:
        """
        Return a deterministic campaign discount.

        Supported rates are 0.1 and 0.2.
        Prefer using customer email as seed_hint; fallback to phone.
        """
    ...
    def calculate_order_totals(
        ...
        *,
        ...
        """
        Validate product IDs, detail token, stock, and discount rate.
        Return an error payload instead of throwing for common user/order issues.
        """
    ...
    def save_order(
        ...
        *,
        ...
        """
        Recompute totals, build deterministic order ID, and persist final JSON.
        """
        missing_fields = []
